...
export const InputForm = () => {
  ...
  const isFormValid = inputId.trim() && label.trim() && placeholder.trim() && isRequired !== null;

  return (
    <>
      <Container width={'100%'} display={'flex'} flexDirection={'column'} gap={5}>
        <Box>
          <Text mb={1} {...styles.labelStyle}>Ingresa un ID único para tu custom input</Text>
          <Input
        ...
        <Box>
          <Text mb={1} {...styles.labelStyle}>Ingresa el nombre de tu placeholder</Text>
          <Input
        ...
        <Box>
          <Text mb={1} fontWeight={'bold'}>¿Es requerido este campo?</Text>
          <Box display="flex" alignItems="center">
            <Text mr={2}>Sí</Text>
            <Checkbox isChecked={isRequired===true} onChange={() => setIsRequired(true)} />
            <Text ml={4} mr={2}>No</Text>
            <Checkbox isChecked={isRequired === false} onChange={() => setIsRequired(false)} />
          </Box>
        </Box>
      </Container>
